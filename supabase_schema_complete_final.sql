...
DROP POLICY IF EXISTS "Owners can view all their pitches" ON pitches;
...
DROP POLICY IF EXISTS "Owners can insert pitches" ON pitches;
CREATE POLICY "Owners can insert pitches" ON pitches
  FOR INSERT WITH CHECK (auth.uid() = owner_id);

-- time_slots
DROP POLICY IF EXISTS "Anyone can view available time_slots" ON time_slots;
CREATE POLICY "Anyone can view available time_slots" ON time_slots
  FOR SELECT USING (status = 'available');

DROP POLICY IF EXISTS "Owners can view all their time_slots" ON time_slots;
...
DROP POLICY IF EXISTS "Owners can update their time_slots" ON time_slots;
...
-- bookings
DROP POLICY IF EXISTS "Players can view their bookings" ON bookings;
CREATE POLICY "Players can view their bookings" ON bookings
  FOR SELECT USING (player_id = auth.uid());

DROP POLICY IF EXISTS "Owners can view bookings for their pitches" ON bookings;
...
DROP POLICY IF EXISTS "Players can insert reviews for their bookings" ON reviews;
CREATE POLICY "Players can insert reviews for their bookings" ON reviews
  FOR INSERT WITH CHECK (player_id = auth.uid());

-- favorites
DROP POLICY IF EXISTS "Players can manage their favorites" ON favorites;
...
-- Example for pitches:
DROP POLICY IF EXISTS "Admins can do everything on pitches" ON pitches;
...
DROP POLICY IF EXISTS "Authenticated users can upload pitch images" ON storage.objects;
...
DROP POLICY IF EXISTS "Users can update and delete their own pitch images" ON storage.objects;
...
DROP POLICY IF EXISTS "Users can update and delete their own avatar" ON storage.objects;
